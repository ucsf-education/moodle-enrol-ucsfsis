...
class enrol_ucsfsis_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $PAGE->requires->yui_module('moodle-enrol_ucsfsis-groupchoosers', 'M.enrol_ucsfsis.init_groupchoosers',
                                    array(array('formid' => $mform->getAttribute('id'), 'courseid' => $course->id)));

        $enrol = $plugin;
        $http  = $plugin->get_http_client();

        $mform->addElement('header','general', get_string('pluginname', 'enrol_ucsfsis'));
        ...
        if ($instance->id) {
            $mform->addElement('select', 'selectschool', get_string('school', 'enrol_ucsfsis'), $schooloptions);
            ...
            $mform->addHelpButton('selectschool', 'school', 'enrol_ucsfsis');
            $mform->registerNoSubmitButton('updateschooloptions');
            $mform->addElement('submit', 'updateschooloptions', get_string('schooloptionsupdate', 'enrol_ucsfsis'));
        }

        if ($instance->id) {
            $mform->addElement('select', 'selectprogram', get_string('program', 'enrol_ucsfsis'), $programoptions);
            ...
            $mform->addHelpButton('selectprogram', 'program', 'enrol_ucsfsis');
            $mform->disabledIf('selectprogram', 'selectschool', 'eq', '');
            $mform->registerNoSubmitButton('updateprogramoptions');
            $mform->addElement('submit', 'updateprogramoptions', get_string('programoptionsupdate', 'enrol_ucsfsis'));
        }

        if ($instance->id) {
            $mform->addElement('select', 'selectcohort', get_string('cohort', 'enrol_ucsfsis'), $cohortoptions);
            ...
            $mform->addHelpButton('selectcohort', 'cohort', 'enrol_ucsfsis');
            $mform->disabledIf('selectcohort', 'selectprogram', 'eq', '');
            $mform->registerNoSubmitButton('updatecohortoptions');
            $mform->addElement('submit', 'updatecohortoptions', get_string('cohortoptionsupdate', 'enrol_ucsfsis'));
        }

        if ($instance->id) {
            $mform->addElement('select', 'selectlearnergroup', get_string('learnergroup', 'enrol_ucsfsis'), $learnergroupoptions);
            ...
            $mform->addElement('select', 'selectlearnergroup', get_string('learnergroup', 'enrol_ucsfsis'), $learnergroupoptions);
            $mform->addHelpButton('selectlearnergroup', 'learnergroup', 'enrol_ucsfsis');
            $mform->disabledIf('selectlearnergroup', 'selectcohort', 'eq', '');
            $mform->registerNoSubmitButton('updatelearnergroupoptions');
            $mform->addElement('submit', 'updatelearnergroupoptions', get_string('learnergroupoptionsupdate', 'enrol_ucsfsis'));
        }

        if ($instance->id) {
            $mform->addElement('select', 'selectsubgroup', get_string('subgroup', 'enrol_ucsfsis'), $subgroupoptions);
            ...
            $mform->addElement('select', 'selectsubgroup', get_string('subgroup', 'enrol_ucsfsis'), $subgroupoptions);
            $mform->addHelpButton('selectsubgroup', 'subgroup', 'enrol_ucsfsis');
        ...
        $mform->addElement('select', 'roleid', get_string('assignrole', 'enrol_ucsfsis'), $roles);
        ...
        $mform->addElement('select', 'customint6', get_string('addgroup', 'enrol_ucsfsis'), $groups);
    ...
    function validation($data, $files) {
        ...
        if ($DB->record_exists_select('enrol', "roleid = :roleid AND customchar1 = :customchar1 AND customint1 = :customint1 AND courseid = :courseid AND enrol = 'ucsfsis' AND id <> :id", $params)) {
            $errors['roleid'] = get_string('instanceexists', 'enrol_ucsfsis');
        }
